...
| Ask | Do this |
|---|---|
| **2026 draft, any pick-level question** | Read the current `out/engine_2026.json`, draft room, and PATHS artifact. Opponent-aware decision cards use survival odds fitted to 2,039 of this league's picks. If freshness requires regeneration, follow the complete `docs/DRAFT_MORNING.md` sequence; `engine_2026.py --slot N` still writes artifacts and must never run alone |
...
| Draft-day app | https://anthonydellapia1117.github.io/yeahthatfantasyleague/out/draft_room.html - or `open out/draft_room.html` offline. Regenerate only through `docs/DRAFT_MORNING.md`: engine, CVS inputs, CVS, and VONA in one pass, then every gate and a branch PR. Never ship an engine-only rebuild or manually push directly to main; the fully gated scheduled refresh is the documented automation exception. Live mode carries the pick grade gear (presentation only, guarded), the ALSO CONSIDER panel on WAIT/COIN FLIP, the Grid screen (12x14 live board), and the Board screen (best-available overall + by position, drafted toggle) |
| Record a conviction call (bull/bear) | Add a row to `data/my_board.csv` (schema + pre-registered scoring rule in its header), then follow the complete `docs/DRAFT_MORNING.md` rebuild and gate sequence. YOUR CALL chips appear beside model VOR; the ONLY decision it moves is a coin-flip tie-break toward bulls. Empty board = byte-identical output (guard 10) |
