...
`backfield_share` used a default zero for draftable RBs without an observed 2025
usage row. In the discovery frame that admitted four false observations, changing
the population from 46 to 50 and the upper-middle median from 0.5060 to 0.4986;
Jacory Croskey-Merritt and Tyler Allgeier moved from criterion probability 0.50 to
0.75. After #65's fresh engine moved Jonah Coleman outside the current 168-pick
pool, the rebuilt artifact has 49 candidates, 46 observations, and three explicit
null exclusions. Every threshold now states `n`, observed-zero count, excluded-null
count, evidence rule, and percentile method. Bucky Irving's observed 0-of-n
inside-five share remains a legitimate zero. The sweep found no second live
contaminated population.

The live TE matrix is suspended. Its advertised 2-of-2 gate had one varying input:
all 19 eligible veterans received the same 0.9 market-share probability, while the
other quantity was pass-block-inclusive on-field dropback share, not routes run.
Isaiah Likely's 2025 Baltimore share was also ranked alone in his 2026 Giants
group. The five former rows - McBride, Warren, Pitts, Kelce, and Henry - are omitted
from live tags but retained in a top-level shadow ledger with explicit resumption
requirements. Big Board, Players, and Draft Room show one neutral artifact-driven
notice; no per-player SUSPENDED chip is invented. The mislabeled live/shadow key is
now `on_field_dropback_share`, and the historical R extract no longer relabels
expected receptions as route participation. Its TE CSV is 6,730 identified rows,
20 columns, and zero NA-subsetting junk rows.
...
count, source SHA, sign convention, top-five teams (LAR, DET, BAL, BUF, SF), and the
actual activation delta. Gains are Brock Purdy and Jordan Love; losses are Caleb
Williams, Jalen Hurts, Justin Herbert, Kyler Murray, and Trevor Lawrence. Patrick
Mahomes moves WATCH to BULLISH; Ja'Marr Chase moves BULLISH to WATCH. All ten RB
...
in the clean regular-season top tier; reproducibility proved arithmetic, not input
fitness. The R Vegas export inverted 224/224 team-games, but the app's existing
Week-1 formula was already correct and the broken CSV is not consumed. Finally,
corrected R source claimed the RB CSV had been regenerated while the committed
artifact still retained `target_volume` and 2,530 junk rows. Correct source and
regenerated bytes are now treated as separate evidence.
