...
python3 src/build_rxr_reference.py       # same engine digest; stale corpus blocks RxR
...
| **Durability fade as a draft signal** | Dropped under a pre-registered rule after investigation. This does not prohibit an owner from using durability outside the model. The complete 4/21/28 policy ranks CMC above Puka by 2.08; a survival sensitivity reverses the order, which establishes rank instability rather than equivalence. Player-action uncertainty is uncalibrated. Anthony's Puka-over-CMC pick-4 choice is an external owner override, not a tie-break authorized by the model and not a new engine signal. |
...
3. `build_rxr_reference.py` and `build_vona_tree.py` AFTER `engine_2026.py`,
   always in the same pass. RxR fails closed when its parity corpus digest is stale.
...
| **Draft order** | **SLEEPER-CONFIRMED 2026-08-31: Anthony is slot 4**, picks 4, 21, 28, 45, 52, 69, 76, 93, 100, 117, 124, 141, 148, 165. Sleeper published 12/12 user assignments plus the complete non-identity roster permutation `{1:10, 2:11, 3:1, 4:7, 5:6, 6:9, 7:3, 8:2, 9:8, 10:5, 11:4, 12:12}`; the resolver source is `draft_order+slot_to_roster_id` and both official sources agree. `start_time=1788912025000` is **2026-09-09 00:00:25 UTC / 2026-09-08 20:00:25 ET**. | Slot 4 is primary on every slot-conditional surface; the other eleven remain reference views. The real confirmed endpoint is observed. Controlled no-reload smoke against the exact captured before/after payloads proves the banner, order card, gap strips, seat chip, and slot-4 geometry repaint together; no browser was demonstrably open through the organic publication moment. Keep the shorter 11-user/12-roster rehearsal branch: it is valid coverage for an ownerless state that did not materialize. Partial, identity, malformed, duplicate, and source-conflict guards remain fail-loud. |
| **Draft-order transition rebuild trigger** | **OPEN AUTOMATION GAP.** `src/check_draft_order.py` and the draw watch detected the pending -> confirmed transition, but neither writes artifacts nor dispatches a build. A human had to notice, run the full dependency chain, and deploy. Until then, byte-identical Pages and matching lineage digests coherently described the old world state. | **LOGGED ONLY - DO NOT BUILD BEFORE THE 2026 DRAFT.** Afterwards, add an idempotent scheduled watcher that dispatches the existing `draft-refresh.yml` with `dry_run=false` only when live reconciliation says `agrees` and the committed engine does not. Never dispatch on conflict or unresolved evidence. Do not duplicate the rebuild chain inside the watcher. The manual path worked and is lower risk for September 8. |
...
| **RxR / full Turn Planner replacement for PATHS** | **BOUNDED PREP SURFACE AUTHORIZED 2026-09-01; full live design remains offseason work. `docs/TURN_PLANNER_SPEC.md` is authoritative.** The old tree has nine nodes and zero rendered forks because it compares aggregate positional VONA from an unconditional pool. It cannot accept "Gibbs, Bijan, Chase gone" or place McCaffrey beside Taylor because both are RBs. Conditioned positional VONA would still render Puka alone. The complete 4/21/28 policy gives McCaffrey a nominal +2.08 over Puka, but player-action uncertainty is uncalibrated. The tree's 7.0 is p25 of 71 current-tree strict-domination margins, not a tie or forecast-error band. PR #54 remains one action per position and does **not** fix either problem. | `RxR PREP` is the sole pre-draft exception: one derived 14-turn rail, one full engine-VOR board, a complete manual/ADP-chalk scenario prefix, and the canonical one-step `score_candidates` mark. It explicitly carries no future survival, tie, forecast, live-poller or optimal-path claim. PATHS remains deployed but unlinked. After the draft, reuse one coherent live-state module, reconcile the Draft Room recommendation, and attempt the mandatory held-out player-action error calibration before calling RxR the full live Turn Planner. Keep #54 parked. |
...
now been observed from Sleeper. Controlled no-reload smoke against the captured
before/after payloads repaints the confirmed banner, order card, gap strips, seat
chip, and slot-4 geometry together; no browser was known to remain open through
the organic publication. This does **not** verify the started-draft path. Until the real draft begins, no live
