...
| G-004 | 2026 | **PARTIALLY RESOLVED 2026-08-31.** External draw reports Anthony at slot 4; Sleeper still exposes an unpublished identity placeholder | Slot 4 is primary, with all eleven other slots retained as reference. Official disagreement would block rather than silently replace the external draw | Close when Sleeper publishes a complete permutation that agrees with slot 4 |
...
| G-007 | 2026 | **RESOLVED AS A NULL PREDICTION.** Written rule 3.3 (draft order = reverse prior finish) is not followed: 17 of 141 slot assignments, 12.1 percent versus 8.3 percent expected at random (z=1.6, not significant). The external draw reports Anthony at slot 4 | The draw was not predictable from standings. The result remains descriptive and does not enter survival or verdicts | Operational slot is known externally; Sleeper confirmation remains tracked under G-004 |
